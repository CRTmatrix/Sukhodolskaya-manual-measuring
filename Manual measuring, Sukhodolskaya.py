import numpy as np,cv2,os,copy,math
...
def measurePoints(event,x,y,flags,param):
        global order,prevX,prevY,table,errorcounter,radius,color,tableRow,erasingsample
        if event==cv2.EVENT_LBUTTONDBLCLK and order<14:
            ...
            if order<=2:
                ...
                for a in range(5,len(tableRow),2):
                    patch=erasingsample[int(tableRow[a])-14:int(tableRow[a])+3,int(tableRow[a+1])-3:int(tableRow[a+1])+14+errorcounter*8]#; patch=np.zeros((17,17,3),'uint8')
                    img[int(tableRow[a])-14:int(tableRow[a])+3,int(tableRow[a+1])-3:int(tableRow[a+1])+14+errorcounter*8]=patch#; print img.shape,len(tableRow),a; cv2.rectangle(img,(int(tableRow[a+1])-3,int(tableRow[a])-14),(int(tableRow[a+1])+14,int(tableRow[a])+3),(0,255,0),3)
                tableRow=tableRow[:5]
        ...
        if Pressed in measureActionKeys:
            if Pressed==ord('q'):
                ...
                if order<14:
                    ...
                    tableRow.append(sex+'\n');table.append(tableRow);writedown(table,directory)
                    order=2
                    tableRow=tableRow[:5]
                    errorcounter=0
