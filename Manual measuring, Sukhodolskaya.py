...
Height=768
Latency=300
Sovdeptriviafractionworkaroundvar=False
...
def Sovdeptriviafractionworkaround(text):
    if Sovdeptriviafractionworkaroundvar==True:
        comatext=''
        for a in text:
            if a !='.':
                comatext+=a
...
def measurePoints(event,x,y,flags,param):
        ...
        if event==cv2.EVENT_LBUTTONDBLCLK:
            ...
            if order<=14:
                tableRow.append(y);tableRow.append(x)
            if order<=2:
                color=(255,90,90)
            elif 2<order<=6:
                color=(0,0,255)
...
def writedown(table,directory):
    ...
    for a in table[1:]:
        ...
        for c in a[5:28:4]:
            d=a.index(c)
            header+=str(math.sqrt((c-a[d+2])**2+(a[d+1]-a[d+3])**2)*10/rowScale)+'\t'
        header+=a[29]
    ...
    with open(directory+'/LM table.txt','w') as results1:
        results1.write(Sovdeptriviafractionworkaround(stringTable))
        results1.close()
    with open(directory+'/Measures table.txt','w') as results2:
        results2.write(Sovdeptriviafractionworkaround(header))
        results2.close()
